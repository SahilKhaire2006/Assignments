# Practical – Faculty Database with Divide Hash Function

**Name:** Sahil Ashok Khaire  
**Roll No.:** 13  
**Title:** Faculty Database Simulation using Divide Hash Function with Linear Probing and Chaining without Replacement  

- Implement a faculty database using hash table
- Use 'divide' as hash function for faculty ID
- Apply linear probing with chaining without replacement for collision resolution
...
A **Faculty Database System** using **hash table** with the **divide hash function** provides efficient storage and retrieval of faculty records. The **divide hash function** uses the formula: `hash = faculty_id % table_size`. **Linear probing with chaining without replacement** is a collision resolution technique where each slot contains the actual record, and collisions are resolved by finding the next available slot while maintaining a chain.

**Key Components**:
- **Hash Table**: Array-based structure for storing faculty records
- **Divide Hash Function**: `hash = faculty_id % table_size`
- **Linear Probing with Chaining**: Collision resolution with explicit chaining
- **Faculty Record**: Contains professional and personal information
- **Chain Pointer**: Links collided records in a chain

**Chaining without Replacement Technique**:
- Each slot contains the actual data record
- When collision occurs, find next available slot using linear probing
- Maintain chain pointers to link records that hashed to same initial index
- Search follows the chain to find the target record

**Advantages**:
- Better performance than simple linear probing
- Maintains data locality
- Efficient search within chains
...
Key terms: **Hash Table**, **Divide Hash Function**, **Linear Probing**, **Chaining without Replacement**, **Faculty Database**, **Collision Resolution**
...
### Algorithm: Divide Hash Function
...
2. If found:
   - Mark slot as empty
   - Update chain pointers to maintain chain integrity
...
## Output
![alt text](Assignment56.png)
![alt text](Assignment56-1.png)