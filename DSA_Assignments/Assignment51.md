...
**Roll No.:** 13  
**Title:** Implementation of Hash Table with Collision Resolution using Linear Probing  
...
- Handle table resizing when load factor exceeds threshold
- Demonstrate collision handling and resolution
...
**Linear Probing** is a collision resolution technique in hash tables. When a collision occurs (i.e., when two keys hash to the same index), linear probing searches for the next available slot in the hash table by sequentially checking the next positions until an empty slot is found.

**Key Concepts**:
- **Hash Function**: Converts keys into array indices
- **Collision**: When two keys hash to the same index
- **Load Factor**: Ratio of number of elements to table size (n/m)
- **Probing Sequence**: i, i+1, i+2, ... modulo table size
...
Key terms: **Hash Table**, **Hash Function**, **Linear Probing**, **Collision Resolution**, **Load Factor**, **Primary Clustering**
...
2. If slot at index is empty or contains deleted marker:
   - Insert key-value pair
   - Increment element count
3. If slot is occupied:
   - Probe linearly: `index_sak = (index_sak + 1) % size_sak`
   - Repeat until empty slot or deleted marker is found
4. If load factor exceeds threshold, resize table

### Algorithm: Search Operation
1. Compute initial index using hash function
2. If slot at index contains the key:
   - Return value and success
3. If slot is empty (not deleted):
   - Return not found
4. If slot is occupied with different key or deleted:
   - Probe linearly: `index_sak = (index_sak + 1) % size_sak`
   - Repeat until key found or empty slot encountered

### Algorithm: Delete Operation
1. Search for the key using search algorithm
2. If key found:
   - Mark slot as deleted (use special marker)
   - Decrement element count
3. If key not found, return error

### Algorithm: Resize Table
1. Create new table with double the size
2. Rehash all valid elements from old table to new table
3. Update table size and threshold
4. Replace old table with new table

### Algorithm: Display Table
1. Iterate through all table slots
2. For each slot:
   - Display index, key, and value
   - Mark empty and deleted slots appropriately
...
## Output
![alt text](Assignment51.png)
![alt text](Assignment51-1.png)